...
prove $\mathrm{diam}(G_N) = O(\sqrt{N})$ with empirical evidence supporting $O(\log N)$,
...
highways with the fewest stops---the bottleneck regime of Lemma~\ref{lem:scarcity}.
...
Moreover, $2p \in (N/2,\, 2N/3]$ places it near the geometric center, and the reduction in
...
$k$-sampling ($k = 1{,}000$) for $N = 10{,}000$ using the
...
The scarcity--accessibility trade-off identified here is not specific
to arithmetic graphs. Any network containing both dense and sparse
subnetworks may exhibit the same structural paradox: low-degree nodes
bridging a dense cluster to a sparse one can dominate betweenness
over high-degree hubs.
This has practical relevance for network resilience analysis,
where standard protection strategies focus on high-degree hubs
while the true vulnerabilities may be low-degree bridge nodes
that are difficult to identify without betweenness computation.
Arithmetic graphs provide a clean, deterministic model in which
this phenomenon can be studied analytically, without the noise
of stochastic network models.
